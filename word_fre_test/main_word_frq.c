...
{	
	...
	if(tmpNode!= NULL)
	{
//		printf("tmpNode:%p  %s\n", tmpNode, tmpNode->word);
		free(tmpNode);
	//	tmpNode = NULL;
//		printf("tmpNode free:%p\n", tmpNode);
	}
}
//处理字符串
...
{	
	...
	while(fscanf(fp, "%64s", tmpword)!= EOF)
	{
		...
		if(afterlen > WORD_LEN)
			continue;
		Node* tmpNode = LinkList_Search(linklist, tmpword , StringCompare);
		...
		{	
			...
			pWordNode->times++;
			LinkList_Insert(linklist ,(Node*)pWordNode ,0);
		}
		else
		{
	...
	{
		//排序打印 按出现次数从小大
		LinkList_Sort_Node(linklist ,CompareNode);			
		LinkList_Traverse_Reverse(linklist, Print);
	}
	else if(bSort == 0)
	{
		//打印出现次数最多的字符串
		LinkList_Sort_Node(linklist ,CompareNode);			
		WordNode* tmp = (WordNode*)LinkList_Get(linklist , linklist->len-1);
		printf("%s  %d\n:", tmp->word, tmp->times);	
	}
	else if(bSort ==2)
	{
		//按词排序 跟词典的一样
		LinkList_Sort_Node(linklist ,CompareNodeWord);			
		LinkList_Traverse_Reverse(linklist, Print);
	}

	LinkList_Free(linklist , FreeNodeMem);
